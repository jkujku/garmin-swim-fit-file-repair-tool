...
public class Decode {
    ...
    * @deprecated This has been deprecated as it does not make sense to have this as a
    *       static method.</br>
    *       Will be Removed in version 17.00 of the FITSDK.</br>
    *       use {@link #getInvalidFileDataSize()} instead
    *
    * @return true if file has an invalid data size
    */
   @Deprecated
   static public boolean getInvalidDataSize() {
    ...
    *       static method.</br>
    *       Will be Removed in version 17.00 of the FITSDK.</br>
    *       use {@link #isFileFit(InputStream)} instead
    *
    * @param value
    *           boolean value to set
    */
   @Deprecated
   public void setInvalidDataSize(boolean value) {
      ...
      try {
         do {
            ...
            for ( ; currentByteIndex < bytesRead; currentByteIndex++ ) {
               decodeReturn = read((byte) buffer[currentByteIndex]);
    ...
    *            static method.</br>
    *             Will be Removed in version 17.00 of the FITSDK.</br>
    *             use {@link #checkFileIntegrity(InputStream)} instead
    *
    * @param in
    *           file input stream
    *
   ...
   static public boolean checkIntegrity(final InputStream in) {
      ...
      try {
         while ( ((data = in.read()) >= 0) && (status == true) ) {
            switch (decode.read((byte) data)) {
               case CONTINUE:
               case MESG:
               case MESG_DEF:
   ...
   public boolean isFileFit(final InputStream in) {
      ...
      try {
         do {
            for ( ; currentByteIndex < bytesRead; currentByteIndex++ ) {
                switch (this.read((byte) buffer[currentByteIndex])) {
                   case CONTINUE:
                   case MESG:
                   case MESG_DEF:
                      break;
      ...
      switch (state) {
         case FILE_HDR:
            switch (fileHdrOffset++) {
               ...
               case 1:
                  ...
                  if ( ( Fit.PROTOCOL_VERSION_MAJOR == FIT_PROTOCOL_VERSION_ONE ) &&
                     ( fileHdrSize != FIT_HEADER_SIZE_NO_CRC ) &&
                     ( fileHdrSize != FIT_HEADER_SIZE_WITH_CRC ) ) {
                      fileBytesLeft += 2;
                      fileHdrOffset = 0;
                      headerException = null;
                      crc = 0;
               ...
               case 7:
                  ...
                  // Check file size
                  if ( (fileDataSize == 0) && ( (invalidDataSize == false) || (invalidFileDataSize == false) ) ) {
         ...
         case FIELD_DATA: {
            ...
            if (fieldBytesLeft == 0) {
               ...
               if (fieldBytesLeft == 0) {
                  fieldBytesLeft = localMesgDefs[localMesgIndex].fields.get(++fieldIndex).getSize();
               }
            }
               ...
               if ((fieldDef.type & Fit.BASE_TYPE_NUM_MASK) < Fit.BASE_TYPES) { // Ignore field if base type not supported.
                  ...
                  if (field != null) {
                     ...
                     if (field.getName().equals("unknown"))
                     {
                        ...
                        System.out.println("mesg\t" + mesg.num + "\tnum \t" + fieldDef.num + "\t" + fieldDef.type);
                     }
